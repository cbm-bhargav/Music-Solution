...
const FooterTopBar = ({ language, isTeacherInfoPage, footerData, blok }) => {
  // ✅ Ensure footerData is always an array
  const safeFooterData = Array.isArray(footerData) ? footerData : [];

  // ✅ Destructure safely
  const [footerLogoSection, ...otherFooterData] = safeFooterData;

  return (
    <div className="contain w-full flex flex-col lg:grid lg:grid-cols-4 lg:gap-6 py-10">
      {footerLogoSection && (
        <FooterLogoSection
          footerData={footerLogoSection}
