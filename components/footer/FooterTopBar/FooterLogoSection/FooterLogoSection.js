...
const FooterLogoSection = ({ footerData = {}, isTeacherInfoPage, language }) => {
  ...
  // Elfsight widget class mapping
  const elfsightClass = isTeacherInfoPage
    ? language === 'ch-en'
      ? 'elfsight-app-4d85e9a6-af0e-482c-9149-1fcf7d897dac'
      : 'elfsight-app-f63d87b7-8f15-4575-ae30-d8bc4a395ab9'
    : language === 'ch-en'
    ? 'elfsight-app-bfb85d97-6cf7-4564-9152-a2356c062327'
    : 'elfsight-app-fd25024a-2452-4e98-9f1a-54ce628a8d66';
  ...
  return (
    <>
      <div className="md:pb-8 lg:basis-1/3 xl:pb-0 lg:w-full lg:max-w-[270px]">
        ...
        {internal_link[0]?.name && (
          <p className="mt-6 text-black text-16px opacity-60">{internal_link[0].name}</p>
        )}

        <div className="lg:flex md:flex mb-8 mt-24">
          <div className="space-y-4">
            {internal_link.slice(0, 2).map((link) => (
              <div key={link._uid}>
                <Link href={link.link.url} passHref>
                  <a className="flex items-center space-x-4 cursor-pointer text-primary">
                    <i className="material-icons-outlined text-20px text-primary">
                      {link.icon_name}
                    </i>
                    <span>{link.name}</span>
                  </a>
      ...
      {/* Load Elfsight script once after interactive */}
      <Script
