  ...
  return (
    <div className="relative z-1">
      <Swiper
        ...
        onSwiper={(swiper) => (swiperRef.current = swiper)}
        className="reviews-slider !pb-2"
      >
        {reviews.map(({ name, image, date, review, rating }, index) => (
          <SwiperSlide key={index}>
            <div className="bg-white border border-gray-300 rounded-xl p-4 text-center flex flex-col items-center h-full w-full max-w-[280px] sm:max-w-full mx-auto">
              ...
              <div className="mt-3 w-[25px] h-[25px] relative">
                <Image
      ...
      {showNav && (
        <>
          <button
            onClick={() => swiperRef.current?.slidePrev()}
            className="absolute left-4 sm:-left-4 top-1/2 -translate-y-1/2 z-10 border border-gray-300 bg-white p-2 rounded-full shadow-md transition-all"
          >
            <ArrowLeft className="w-[20px] h-[20px]" />
          </button>
          <button
            onClick={() => swiperRef.current?.slideNext()}
            className="absolute right-4 sm:-right-4 top-1/2 -translate-y-1/2 z-10 border border-gray-300 bg-white p-2 rounded-full shadow-md transition-all"
          >
            <ArrowLeft className="w-[20px] h-[20px] rotate-180" />
          </button>
        </>
