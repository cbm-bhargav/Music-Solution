...
function EventsData({ language, organizationData }) {
  const eventData = organizationData?.events?.[0];
  const eventImage = eventData?.image || "/assets/images/logo.png";

  return (
    <div
      className={cx(
        "teacher-content-block md:rounded-xl px-4 py-5 sm:p-5 w-full mb-3 sm:mb-5"
      )}
    >
      <div className="flex flex-col items-start justify-between">
        <h3 className="font-bold text-[17px] sm:text-[19px] leading-[126%] font-Roboto mb-5 sm:mb-0">
          {translateENtoDE("Events", language)}
        </h3>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-5">
            ...
            <div>
              ...
              <div className="[&>div>p]:!text-[14px]">
                ...
                >
                  {language === "ch-en"
                    ? eventData?.event_text?.en
                    : eventData?.event_text?.de}
                </ShowMoreTextNext>
              </div>
            </div>
