...
const Form = ({ blok, close }) => {
  ...
  // console.log('Inpura: ', blok?.inputs);
  ...
  return (
    <form
      className='w-full rounded-xl space-y-4'
      {...storyblokEditable(blok)}
      >
      {blok.inputs.map((nestedBlok) => (
        <DynamicComponent blok={nestedBlok} key={nestedBlok._uid} register={register} errors={errors} />
      ))}
      <button
        style={{
          borderRadius: 30,
